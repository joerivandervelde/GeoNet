...
def eval_metrics(probs, targets, cal_AUC=True, cal_PRC=True):
    ...
    for threshold in threshold_list:
        threshold, rec, pre, F1, spe, mcc, _, _, pred_bi = th_eval_metrics(threshold, probs, targets, cal_AUC=False)
        if F1 > F1_best:
            threshold_best, rec_best, pre_best, F1_best, spe_best, mcc_best, pred_bi_best = threshold, rec, pre, F1, spe, mcc, pred_bi

    return threshold_best, rec_best, pre_best, F1_best, spe_best, mcc_best, roc_auc, prc_auc, pred_bi_best
...
def th_eval_metrics(threshold, probs, targets, cal_AUC=True, cal_PRC=True):
    ...
    return threshold, rec, pre, F1, spe, mcc, auc_, prc_, pred_bi
