...
class MessagePassing(torch.nn.Module):
    ...
    def __collect__(self, edge_index, size, kwargs):
        ...
        i, j = (1, 0) if self.flow == "target_to_source" else (0, 1)
        ij = {"_i": i, "_j": j}
    ...
    def propagate(self, edge_index, size=None, **kwargs):
        ...
        out, alpha, edge_attr = self.message(**msg_kwargs)

        aggr_kwargs = self.__distribute__(self.__aggr_params__, kwargs)
        out, u = self.aggregate(out, **aggr_kwargs)

        update_kwargs = self.__distribute__(self.__update_params__, kwargs)
        out = self.update(out, **update_kwargs)


        return out, u, alpha, edge_attr
    ...
    def aggregate(self, inputs, index, dim_size):  # pragma: no cover
        ...
        """
        # x = scatter_add(inputs, index, self.node_dim)
        # u = scatter_add(x, batch, self.node_dim)
        # return x, u
        return inputs

    def update(self, inputs, u):  # pragma: no cover
